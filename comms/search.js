    ...
    async execute(message, args) {
        ...
        function wrongSyntax() {
            ...
            con.connect(error => {
                ...
                con.query("SELECT m.attacker AS 'Steam_ID', `Name`, `Wounds`,`Kills`,`Deaths`,`Kills`/`Deaths` AS `K/D`,`Revives`,m.id AS 'ID', `TeamKills`, `Favorite Gun`, `Favorite Role` FROM `DBLog_Wounds` m LEFT JOIN ( SELECT attacker, COUNT(*) AS `Wounds`, `match` FROM `DBLog_Wounds` WHERE server IN (" + SETTINGS.serverID + ") AND `match` NOT IN (SELECT id FROM squadjs.DBLog_Matches WHERE map IN (" + SETTINGS.ignoredMaps + ")) GROUP BY attacker ORDER BY time ASC) w ON w.attacker = m.attacker LEFT JOIN (SELECT attacker, COUNT(*) AS `TeamKills`, `match` FROM `DBLog_Deaths` WHERE server IN (" + SETTINGS.serverID + ") AND `match` NOT IN (SELECT id FROM squadjs.DBLog_Matches WHERE map IN (" + SETTINGS.ignoredMaps + ")) AND teamkill = 1 GROUP BY attacker) tk ON tk.attacker = m.attacker JOIN (SELECT attacker, weapon AS `Favorite Gun`, COUNT(weapon), `match` FROM DBLog_Wounds WHERE attacker = " + a + " AND `match` NOT IN (SELECT id FROM squadjs.DBLog_Matches WHERE map IN (" + SETTINGS.ignoredMaps + ")) GROUP BY weapon ORDER BY COUNT(weapon) DESC) fw ON fw.attacker = m.attacker JOIN (SELECT attacker, weapon AS `Favorite Role`, COUNT(weapon), `match` FROM DBLog_Deaths WHERE attacker = " + a + " AND `match` NOT IN (SELECT id FROM squadjs.DBLog_Matches WHERE map IN (" + SETTINGS.ignoredMaps + ")) GROUP BY weapon ORDER BY COUNT(weapon) DESC) fg ON fg.attacker = m.attacker LEFT JOIN (SELECT attacker, COUNT(*) AS `Kills`, `match` FROM `DBLog_Deaths` WHERE server IN (" + SETTINGS.serverID + ") AND `match` NOT IN (SELECT id FROM squadjs.DBLog_Matches WHERE map IN (" + SETTINGS.ignoredMaps + ")) GROUP BY attacker) k ON k.attacker = m.attacker LEFT JOIN ( SELECT victim, COUNT(*) AS `Deaths`, `match` FROM `DBLog_Deaths` WHERE server IN (" + SETTINGS.serverID + ") AND `match` NOT IN (SELECT id FROM squadjs.DBLog_Matches WHERE map IN (" + SETTINGS.ignoredMaps + ")) GROUP BY victim) d ON d.victim = m.attacker LEFT JOIN (SELECT steamID, lastName AS `Name` FROM `DBLog_SteamUsers`) s ON s.steamID = m.attacker LEFT JOIN ( SELECT reviver, COUNT(*) AS `Revives`, `match` FROM `DBLog_Revives` WHERE server IN (" + SETTINGS.serverID + ") AND `match` NOT IN (SELECT id FROM squadjs.DBLog_Matches WHERE map IN (" + SETTINGS.ignoredMaps + ")) GROUP BY reviver ) r ON r.reviver = m.attacker WHERE steamID = " + a + " AND server IN (" + SETTINGS.serverID + ") GROUP BY m.attacker HAVING `K/D` IS NOT NULL ORDER BY `K/D` DESC, time DESC", function (error, result, fields) {
                    if (error) {
                    ...
                    if (!result || result.length === 0) {
                        ...
                        steam.getUserSummary(a).then(summary => {
                            ...
                            searchEmbed.addField(
                                `**Favorite Role**`,
                                `\`\`\`${result[0]["Favorite Role"]}\`\`\``,
                                false
                            );

                            searchEmbed.addField(
                                `**Favorite Gun**`,
                                `\`\`\`${result[0]["Favorite Gun"]}\`\`\``,
                                false
                            );
